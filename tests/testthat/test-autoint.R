...
test_that("autoint regression - formula interface", {
  ...
  set.seed(1)
  fit <- brulee_auto_int(y ~ ., data = df, epochs = 5, verbose = FALSE)
...
test_that("autoint regression - data.frame interface (numeric only)", {
  ...
  set.seed(1)
  fit <- brulee_auto_int(x = x, y = y, epochs = 3, verbose = FALSE)
...
test_that("autoint regression - recipe interface", {
  ...
  set.seed(1)
  fit <- brulee_auto_int(rec, data = df, epochs = 3, verbose = FALSE)
