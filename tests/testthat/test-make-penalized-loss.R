test_that("make_penalized_loss returns unmodified loss for ADAMw", {
  ...
  loss_fn_adamw <- brulee:::make_penalized_loss(base_loss_fn, model, penalty = 1, mixture = 0, opt = "ADAMw")
...
test_that("make_penalized_loss returns unmodified loss for pure L2 with non-LBFGS optimizers", {
  ...
  for (opt in optimizers) {
    loss_fn <- brulee:::make_penalized_loss(base_loss_fn, model, penalty = 1, mixture = 0, opt = opt)
...
test_that("make_penalized_loss adds penalty to loss for L1 (mixture = 1)", {
  ...
  loss_fn_l1 <- brulee:::make_penalized_loss(base_loss_fn, model, penalty = 0.1, mixture = 1, opt = "SGD")
...
test_that("make_penalized_loss adds penalty to loss for elastic net", {
  ...
  loss_fn_en <- brulee:::make_penalized_loss(base_loss_fn, model, penalty = 0.1, mixture = 0.5, opt = "SGD")
...
test_that("make_penalized_loss LBFGS always adds penalty to loss", {
  ...
  loss_fn_lbfgs_l2 <- brulee:::make_penalized_loss(base_loss_fn, model, penalty = 0.1, mixture = 0, opt = "LBFGS")
  ...
  loss_fn_lbfgs_l1 <- brulee:::make_penalized_loss(base_loss_fn, model, penalty = 0.1, mixture = 1, opt = "LBFGS")
  ...
  loss_fn_lbfgs_en <- brulee:::make_penalized_loss(base_loss_fn, model, penalty = 0.1, mixture = 0.5, opt = "LBFGS")
...
test_that("make_penalized_loss penalty = 0 returns unmodified loss", {
  ...
  loss_fn <- brulee:::make_penalized_loss(base_loss_fn, model, penalty = 0, mixture = 1, opt = "SGD")
...
test_that("make_penalized_loss different mixture values produce different penalties", {
  ...
  # L2 only (mixture = 0)
  loss_fn_l2 <- brulee:::make_penalized_loss(base_loss_fn, model, penalty = 1, mixture = 0, opt = "SGD")
  loss_l2 <- loss_fn_l2(input, target)

  # L1 only (mixture = 1)
  loss_fn_l1 <- brulee:::make_penalized_loss(base_loss_fn, model, penalty = 1, mixture = 1, opt = "SGD")
  loss_l1 <- loss_fn_l1(input, target)

  # Elastic net (mixture = 0.5)
  loss_fn_en <- brulee:::make_penalized_loss(base_loss_fn, model, penalty = 1, mixture = 0.5, opt = "SGD")
  loss_en <- loss_fn_en(input, target)

  # All three should be different
  expect_false(isTRUE(all.equal(as.numeric(loss_l2$item()), as.numeric(loss_l1$item()))))
  expect_false(isTRUE(all.equal(as.numeric(loss_l2$item()), as.numeric(loss_en$item()))))
  expect_false(isTRUE(all.equal(as.numeric(loss_l1$item()), as.numeric(loss_en$item()))))
})
...
test_that("make_penalized_loss higher penalty produces higher loss", {
  ...
  # Small penalty
  loss_fn_small <- brulee:::make_penalized_loss(base_loss_fn, model, penalty = 0.1, mixture = 1, opt = "SGD")
  loss_small <- loss_fn_small(input, target)

  # Large penalty
  loss_fn_large <- brulee:::make_penalized_loss(base_loss_fn, model, penalty = 10, mixture = 1, opt = "SGD")
...
test_that("make_penalized_loss works with different optimizers", {
  ...
  for (opt in optimizers) {
    # Should not error
    loss_fn <- brulee:::make_penalized_loss(base_loss_fn, model, penalty = 0.1, mixture = 0, opt = opt)
