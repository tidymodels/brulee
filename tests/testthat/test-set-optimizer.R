...
test_that("set_optimizer sets weight_decay to 0 for elastic net (0 < mixture < 1)", {
  ...
  opt_sgd <- brulee:::set_optimizer("SGD", model, 0.1, 0.9, penalty_val, mixture = 0.5)
  expect_equal(opt_sgd$defaults$weight_decay, 0)

  # RMSprop with mixture = 0.3 should NOT use weight_decay
  opt_rmsprop <- brulee:::set_optimizer("RMSprop", model, 0.1, 0.9, penalty_val, mixture = 0.3)
  expect_equal(opt_rmsprop$defaults$weight_decay, 0)

  # Adadelta with mixture = 0.7 should NOT use weight_decay
  opt_adadelta <- brulee:::set_optimizer("Adadelta", model, 0.1, 0.9, penalty_val, mixture = 0.7)
  expect_equal(opt_adadelta$defaults$weight_decay, 0)
})
...
test_that("set_optimizer ADAMw always uses weight_decay", {
  ...
  # ADAMw with mixture = 0 should use weight_decay
  opt_adamw_0 <- brulee:::set_optimizer("ADAMw", model, 0.1, 0.9, penalty_val, mixture = 0)
  ...
  opt_adamw_05 <- brulee:::set_optimizer("ADAMw", model, 0.1, 0.9, penalty_val, mixture = 0.5)
  expect_equal(opt_adamw_05$defaults$weight_decay, penalty_val)

  # ADAMw with mixture = 1 should use weight_decay
  opt_adamw_1 <- brulee:::set_optimizer("ADAMw", model, 0.1, 0.9, penalty_val, mixture = 1)
...
test_that("set_optimizer weight_decay with penalty = 0", {
  ...
  opt_sgd <- brulee:::set_optimizer("SGD", model, 0.1, 0.9, penalty = 0, mixture = 0)
  expect_equal(opt_sgd$defaults$weight_decay, 0)

  opt_adamw <- brulee:::set_optimizer("ADAMw", model, 0.1, 0.9, penalty = 0, mixture = 0)
  expect_equal(opt_adamw$defaults$weight_decay, 0)

  opt_rmsprop <- brulee:::set_optimizer("RMSprop", model, 0.1, 0.9, penalty = 0, mixture = 0)
  expect_equal(opt_rmsprop$defaults$weight_decay, 0)
})
