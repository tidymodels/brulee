test_that("scheduling functions", {
  ...
  expect_true(all(
    map_dbl(x[x %% 20 == 0], schedule_cyclic, step_size = 10) == 0.001
  ))
  ...
  expect_equal(
    ...
    map_dbl(
      x,
      ~ set_learn_rate(.x, 0.1, "decay_time", initial = 1 / 3, decay = 7 / 8)
    )
  )
  ...
  expect_equal(
    ...
    map_dbl(
      x,
      ~ set_learn_rate(.x, 0.1, "decay_expo", initial = 1 / 3, decay = 7 / 8)
    )
  )
