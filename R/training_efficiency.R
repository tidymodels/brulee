...
#'
#' GPUs can perform calculations very fast, sometimes faster than the overhead
#' of a high-level interface such as brulee. GPU utilization might be lower than
#' expected because the model is not very large (i.e., with millions of
#' parameters) and/or because the batch size is small.
#'
#' For the latter, here is an example of a training set with 1K samples, one
#' single hidden layer with 50 units, 200 epochs, and used ADAMw optimizer:
#'
#' ```
#'                                          (CPU/CUDA)
