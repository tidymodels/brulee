...
#' @return
#'
#' A `torch_linear_reg` object with elements:
...
#' @export
#' @rdname torch_linear_reg
torch_linear_reg.matrix <- function(x,
                             y,
                             epochs = 100L,
                             penalty = 0,
                             validation = 0.0,
...
torch_linear_reg_bridge <- function(processed, epochs,
                             learning_rate, penalty, validation,
...
new_torch_linear_reg <- function( models, loss, dims, parameters, blueprint) {
 if (!is.list(models)) {
...
torch_linear_reg_reg_fit_imp <-
 function(x, y,
  ...
  list(
   ...
   dims = list(p = p, n = n),
   parameters = list(learning_rate = learning_rate,
...
#' @export
coef.torch_linear_reg <- function(object, epoch = NULL, ...) {
  if (is.null(epoch)) {
    epoch <- length(object$models)
  }
 module <- revive_model(object, epoch = epoch)
 parameters <- module$parameters
 res <- c(as.array(parameters$fc1.bias), as.array(parameters$fc1.weight))
 res <- as.vector(res)
 names(res) <- c("(Intercept)", names(object$blueprint$ptypes$predictors))
 res
}
