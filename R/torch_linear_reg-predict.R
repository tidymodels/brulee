...
#'
#' @export
predict.torch_linear_reg <- function(object, new_data, type = "numeric", epoch = NULL, ...) {
 forged <- hardhat::forge(new_data, object$blueprint)
 type <- check_type(object, type)
 if (is.null(epoch)) {
  epoch <- length(object$models)
 }
 predict_torch_linear_reg_bridge(type, object, forged$predictors, epoch = epoch)
}

lr_valid_predict_types <- function() {
 "numeric"
}
...
predict_torch_linear_reg_bridge <- function(type, model, predictors, epoch) {
 ...
 predict_function <- get_lr_predict_function(type)
...
get_lr_predict_function <- function(type) {
 switch(
  type,
...
predict_torch_linear_reg_raw <- function(model, predictors, epoch) {
...
predict_torch_linear_reg_numeric <- function(model, predictors, epoch) {
 predictions <- predict_torch_linear_reg_raw(model, predictors, epoch)
