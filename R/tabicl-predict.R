...
# the requested statistic of the predictive distribution (regression).
...
#'   - `"numeric"` for the mean of the predictive distribution (regression).
#'   - `"quantile"` for quantiles of the predictive distribution (regression).
...
#'   interval `(0, 1)`, sorted and unique. Only used when `type = "quantile"`.
#'   Defaults to `(1:9) / 10`.
...
#' distribution, so `quantile_levels` does not change what the model computes,
#' only which values are read off afterwards. Any level in the open interval
#' `(0, 1)` is available, including levels far enough into the tails to be
#' extrapolated.
#'
#' With `num_estimators > 1`, each ensemble member yields its own distribution
...
#'
#' Two consequences are worth stating plainly. First, `.pred` is the mean of the
#' predictive distribution, so it is not the same as the 0.5 entry of
#' `.pred_quantile`; this differs from [predict.brulee_chronos()], where `.pred`
#' is defined to be the median. Second, `.pred_variance` pools each member's own
#' variance and so does not include the spread between members' central
#' estimates; it will not equal the variance implied by `.pred_quantile`.
#'
...
#' otherwise there is a single prediction column: `.pred_class` for `"class"`,
#' `.pred` for `"numeric"`, `.pred_variance` for `"variance"`, and
#' `.pred_quantile` for `"quantile"` (a [hardhat::quantile_pred()] vector
...
#' @export
predict.brulee_tab_icl <- function(
  object,
  new_data,
  type = NULL,
  ...
  type <- check_type(
    object,
    type,
    numeric_types = c("numeric", "quantile", "variance"),
    call = call
  )
  # Checked before the checkpoint is reloaded and the ensemble runs, so a bad
  # level fails immediately rather than after the expensive part.
  ...
  if (object$task == "classification") {
    ...
    predictions <- switch(
      ...
      quantile = tibble::tibble(
        .pred_quantile = hardhat::quantile_pred(est$quantiles, quantile_levels)
      ),
      variance = tibble::tibble(.pred_variance = est$variance)
    )
  }
